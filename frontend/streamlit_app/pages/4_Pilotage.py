...
from core.auth import get_current_user, require_backoffice_access
...
def load_tickets_df() -> pd.DataFrame:
    ...
    if "ticket_maitre_id" not in df.columns:
        df["ticket_maitre_id"] = pd.NA
    ...
    if "assigne_a" not in df.columns:
        df["assigne_a"] = None

    if "demandeur" not in df.columns:
        df["demandeur"] = "Inconnu"

    if "typage" not in df.columns:
        df["typage"] = "Non renseigné"

    if "site" not in df.columns:
        df["site"] = "Non renseigné"
...
with f1:
    filtre_type = st.selectbox(
        "Type",
        ["Tous"] + sorted(df["typage"].dropna().astype(str).unique().tolist())
    )
...
with f3:
    priorites = sorted([p for p in df["priorite"].dropna().astype(str).unique().tolist() if p])
...
if filtre_statut != "Tous":
    if filtre_statut == "Doublon":
        filtered = filtered[filtered["is_duplicate_child"]]
    else:
...
duplicate_count = int(filtered["is_duplicate_child"].sum())
...
resolution_series = resolved_df["resolution_hours"].dropna()

mean_resolution_h = round(resolution_series.mean(), 1) if not resolution_series.empty else None
median_resolution_h = round(resolution_series.median(), 1) if not resolution_series.empty else None
...
for titre_item in filtered["titre"].fillna(""):
    for word in str(titre_item).lower().replace("/", " ").replace("-", " ").split():
        cleaned = word.strip(" ,.;:!?()[]{}'\"")
...
sort_column = "created_at" if "created_at" in filtered.columns else "id"
