...
from core.config import PRIORITES, TYPES
from core.styles import apply_global_styles, render_header
from core.tickets import get_tickets, get_ticket, get_comments, get_logs, add_comment
...
# - ou les tickets qui lui sont assignés
if not df.empty:
    df = df[
    ...
    if selected_ticket_id is None or selected_ticket_id not in filtered["id"].tolist():
        ...
        if ticket:
            ...
            new_comment = st.text_area(
                ...
                key=f"comment_my_ticket_{selected_ticket_id}",
            )
